...
from folium.plugins  import MeasureControl, Fullscreen, MarkerCluster, BeautifyIcon, Search             #Для вывода инструментов на карте
from geojson import Feature, FeatureCollection, Point, LineString                                       #Для работы с данными GeoJSON
import json

############################################################################################################################################
#Создание карты с центром в координатах.
map = folium.Map(location = [59.377796, 38.737030], #Координаты Шекснинского РЭС
...
#Изменение цвета в зависимости от номинального напряжения согласно Приказу Минэкономразвития № 10 от 09.01.2018, стр. 124-126
...
features_gen = data_gen.apply(
    lambda row: Feature(properties = {'name':row['Название ЭС'],
                                      'address':row['Место расположения, адрес']},
                        geometry = Point((float(row['Долгота ЭС']),
                                          float(row['Широта ЭС'])))),
                                          axis = 1).tolist()

feature_collection_gen = FeatureCollection(features = features_gen)

with open('gen.geojson', 'w', encoding='cp1251') as f:
  json.dump(feature_collection_gen, f, ensure_ascii = False)

#Создание слоев для подстанций и электростанций
...
#Создание маркеров
marker_cluster = MarkerCluster(name = 'Объекты').add_to(map)
